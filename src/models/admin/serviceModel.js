...
const Service = {
  create: async (title, description, email_description, short_code, group, sac_code, admin_id, callback) => {
    try {
      ...
      const [results] = await sequelize.query(insertServiceSql, {
        replacements: [title, description, email_description, short_code, group, sac_code, admin_id],
  ...
  getServiceById: async (id, callback) => {
    try {
      ...
      const results = await sequelize.query(sql, {
        replacements: [id],
        type: sequelize.QueryTypes.SELECT, // Ensures we only retrieve data
      });
  ...
  getServiceRequiredDocumentsByServiceId: async (service_id, callback) => {
    try {
      ...
      const results = await sequelize.query(sql, {
        ...
        type: sequelize.QueryTypes.SELECT, // Ensures data retrieval
      });
  ...
  update: async (id, title, description, email_description, short_code, sac_code, callback) => {
    try {
      ...
      const [results] = await sequelize.query(updateSql, {
        replacements: [title, description, email_description, short_code, sac_code, id],
